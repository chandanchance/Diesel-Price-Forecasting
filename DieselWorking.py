# -*- coding: utf-8 -*-
"""
Created on Mon Jan  1 10:50:55 2019
...
considerLast = 5
neurons = 50
trainTestCutOffAt = 24

# Importing the dataset

dataset = pd.read_csv("DieselData.csv")
dataset = dataset[100:]
targetCol = "Diesel Price (Indian Rupee per Gallon)"
dataset = dataset[["Month",targetCol]]
#dataset = dataset["Diesel ROC(%)"].as
dataset[targetCol] = pd.to_numeric(dataset[targetCol])
...
forecast = []
data =dataset_test.iloc[:, 1:2].values
originalData = data
data = np.append(data,[np.mean(data)])
data = data.reshape(-1,1)
sc = MinMaxScaler(feature_range = (0, 1))
...
plt.plot(testing_set[considerLast:], color = 'red', label = 'Actual')
plt.plot(values, color = 'blue', label = 'Predicted')
plt.title('CrudeOil')
plt.xlabel('Time')
plt.ylabel('Value')
plt.legend()
